<?php
require_once 'core/init.php';


$user = new User();
$info_curso = "";
if(($user->isLoggedIn())){
    if(!$user->hasPermission('admin')){
        $info_curso = Director_curso::getDadosDirector($user->data()->id);
    }
    $trabalho = Trabalho::find3($info_curso->id_curso);
    //var_dump($trabalho);
    //die();
	?>



<html lang="pt">
<head>
<meta charset="utf-8">
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<title></title>
	<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
	<link rel="stylesheet" type="text/css" href="CSS/estudantReg.css"><link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.10.3/font/bootstrap-icons.css">
    <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@20..48,100..700,0..1,-50..200" />
    <link href="https://fonts.googleapis.com/icon?family=Material+Icons">
    <link href="bootstrap-5.0.2-dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
    <link href="fontawsome/css/all.min.css" rel="stylesheet">
    <link href="fontawsome/css/fontawesome.min.css" rel="stylesheet">
    <link rel="stylesheet" href="css/index-style.css">
    <link rel="stylesheet" href="css/form-style.css">
    <script src="https://kit.fontawesome.com/c4254e24a8.js" crossorigin=""></script>

    <script src="Jquery/jq.js"></script>

    
    <style>
     .tb-data tr:nth-child(even){background-color: #a4bdc9;}

    .tb-data tr:hover {background-color: rgb(154, 186, 215);}


    .tb-data {
        font-family: Arial, Helvetica, sans-serif;
        border-collapse: collapse;
        width: 70%;
        margin: 20px;
    }

    .tb-data th{
        padding-top: 12px;
        padding-bottom: 12px;
        text-align: left;
        background-color: rgb(16, 169, 234);
        color: white;
    }

    .tb-data td, #dados th {
        border: 1px solid #ddd;
        padding: 8px;
    }

    .tb-data tr td button:hover{
        color: aqua;
        transition: 0.2s ease-in;
    }
    </style>

</head>

<body>
	<div class="menu">
    <div class="home">
			<a href="index.php"><i class="fa-solid fa-house"></i></a>
		</div>
        <!--<div class="division"></div>-->
        <div>
            <h2>Sistema de Gestão de Trabalhos de Culminação de Curso </h2>
        </div>
        <div class="division"></div>
        <div>
            <p>Username: Agostinho Ngonga</p>
        </div>
        <div class="division"></div>
        <div>
            <i id="logout" class="bi bi-box-arrow-left"></i>
        </div>
    </div>
	<main class="main">
        
        <input type="hidden" name="curso" id="curso" value="<?php echo $info_curso->id_curso; ?>">
        <div class="row">
            <div class="form-floating mb-3">
                <input type="text" name="docente" class="form-control" id="docente" placeholder="Nome">
                <label for="floatingInput">nome do docente</label>
                <ul class="list"></ul>
            </div>
            </div>
            <div class="row">
            <div class="form-floating mb-3">
                <button class="btn btn-primary" onclick="getTrabalhos()">buscar</button>
            </div>
            </div>
			<input type="hidden" name="codigo_docente" id="codigo_docente">
            
		<table class="tb-data">
            <thead>
                <tr><th>Tema</th><th>Nome estudante</th><th>estado</th><th>regime</th></tr>
            </thead>
            <tbody id="tbody">
               
            </tbody>
        </table>

	</main>
    <script src="search_docentes2.js"></script>
</body>
</html>
<script>
    
    function getTrabalhos(){
        
        console.log(document.getElementById("codigo_docente").value);

        var ajax = new XMLHttpRequest();
        var method = "GET";
        var url = "api_get_trabalhos_docente.php?codigo_docente="+document.getElementById("codigo_docente").value;
        var asynchronous = true;
        ajax.open(method, url, asynchronous); 
        ajax.send();
        ajax.onreadystatechange = function(){

            if(this.readyState == 4 && this.status == 200){
                var trabalho = JSON.parse(this.responseText);
                console.log(trabalho);
            
                //select = document.getElementById("trabalho");
                var html = "";
                for(var a = 0; a< trabalho.length; a++){
                    if(trabalho[a].codigo_docente="null"){
                        html += "<tr><td>"+trabalho[a].tema+"</td><td>"+trabalho[a].nome_estudante+"</td><td>"+trabalho[a].estado+"</td><td>"+trabalho[a].regime+"</td></tr>"
                    }
                    
                }
                document.getElementById("tbody").innerHTML = html;
                }else{
                    console.log("erro");
                }
        }
    }
</script>
<?php
}else{
    Redirect::to("index.php");
}